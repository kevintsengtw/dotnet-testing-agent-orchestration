...
從使用者輸入提取目標服務或 AppHost 的名稱，執行：
`$cn = "{TargetName}"; New-Item -ItemType Directory -Force -Path ".orchestrator/$cn" | Out-Null`

**2. 記錄工作流程開始時間**

`$logFile = "aspire-orchestrator-timing.log"; Add-Content -Path $logFile -Value "WORKFLOW_START $(Get-Date -Format 'yyyy-MM-ddTHH:mm:ss')"`
...
8. **JSON 交接路徑** — 告知 Writer 從 `.orchestrator/{TargetName}/analyzer-result.json` 讀取完整分析報告；完成後寫入摘要至 `.orchestrator/{TargetName}/writer-result.json`
...
在工作流程執行過程中，使用 `execute/runInTerminal` 於指定時間點記錄 timestamp（計時記錄檔：`aspire-orchestrator-timing.log`）：

| 時間點 | 記錄內容 |
|--------|----------|
| 工作前置完成後，開始委派前 | `WORKFLOW_START` |
| 委派 Aspire Analyzer 前 | `PHASE_1_START` |
| 收到 Analyzer 結果後 | `PHASE_1_END` |
| 委派 Aspire Writer 前 | `PHASE_2_START` |
| 收到 Writer 結果後 | `PHASE_2_END` |
| 委派 Aspire Executor 前 | `PHASE_3_START` |
| 收到 Executor 結果後 | `PHASE_3_END` |
| 委派 Aspire Reviewer 前 | `PHASE_4_START` |
...
8. **完整程式碼顯示條件**：只有在使用者明確要求「顯示完整測試程式碼」時，才輸出完整程式碼
