...
| 動作時機 | 必輸出文字 |
|---------|----------|
| 委派 Integration Analyzer **前** | `## 階段 1：委派分析（Integration Analyzer）` |
| Analyzer 回傳後 | `Analyzer 分析完成！識別出 N 個端點、資料庫：[類型]、容器需求：[清單]。現在委派 Integration Writer 撰寫測試。` |
| 委派 Integration Writer **前** | `## 階段 2：委派撰寫（Integration Writer）` |
| Writer 回傳後 | `Writer 完成！已建立整合測試基礎設施與測試類別，共 N 個測試案例。現在委派 Integration Executor 建置與執行。` |
